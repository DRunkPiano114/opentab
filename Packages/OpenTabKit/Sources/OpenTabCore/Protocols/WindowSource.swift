...
public protocol WindowSource: Sendable {
    ...
    /// returns a partial list as success.
...
public protocol WindowActivator: Sendable {
    ...
    /// back, never by an AppKit return value.
...
public protocol AppDirectory: Sendable {
    ...
    /// `NSRunningApplication.isHidden`. AppKit lags the window server, so this
    /// is a secondary sort signal and an input to the activation path, never a
    /// reason to hide rows.
